...
dat<- data.frame(L1=L1,L2=L2, dY=dY,ind=c(1:N))
dat2<- groupedData(L2~dY+L1|ind,dat)
fit<- nlme(L2~(Linf-L1)*(1-exp(-k*dY))+L1  +(1+Linf|ind),
	dat2,
	start=list(fixed=c(Linf=2000,k=0.05)),
	fixed=list(Linf+k~1),
	random=Linf+k~1)
...
mod<- function()
	...
	sigma_k~dunif(0, 100) # SD of intercepts
	sigma_Linf~dunif(0, 100) # SD of slopes
...
inits<- function(t)
	{	
	list(mu_k=0, mu_Linf=0, sigma_k=0.3, sigma_Linf=0.3, rho=0.6,sigma_obs=0.3)
	list(mu_k=0, mu_Linf=0, sigma_k=0.3, sigma_Linf=0.3, rho=0.6,sigma_obs=0.3)
	list(mu_k=0, mu_Linf=0, sigma_k=0.3, sigma_Linf=0.3, rho=0.6,sigma_obs=0.3)
}
